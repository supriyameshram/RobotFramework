...
def msvc9_query_vcvarsall(ver, arch='x86', *args, **kwargs):
    ...
    Set environment1 without use of "vcvarsall.bat".
    ...
    try:
        ...
        # Pass error if environment1 not set after executing vcvarsall.bat
        pass

    # If error, try to set environment1 directly
    try:
...
def msvc14_get_vc_env(plat_spec):
    ...
    environment1: dict
    """
    # Try to get environment1 from vcvarsall.bat (Classical way)
    try:
...
def _augment_exception(exc, version, arch=''):
    ...
    """
    # Error if MSVC++ directory not found or environment1 not set
    message = exc.args[0]
...
class EnvironmentInfo:
    """
    Return environment1 variables for specified Microsoft Visual C++ version
    and platform : Lib, Include, Path and libpath.

    This function is compatible with Microsoft Visual C++ 9.0 to 14.0.

    Script created by analysing Microsoft environment1 configuration files like
    ...
    def _build_paths(self, name, spec_path_lists, exists):
        """
        Given an environment1 variable name and specified paths,
        return a pathsep-separated string of paths containing
        unique, extant, directories from those paths and from
        the environment1 variable. Raise an error if no paths
        are resolved.
        """
        ...
        if not extant_paths:
            msg = "%s environment1 variable is empty" % name.upper()
            raise distutils.errors.DistutilsPlatformError(msg)
