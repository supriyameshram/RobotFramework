...
class OperatingSystem(object):
    ...
    manipulate environment1 variables (e.g. `Set Environment Variable`).
    ...
    def get_environment_variable(self, name, default=None):
        """Returns the value of an environment1 variable with the given name.

        If no such environment1 variable is set, returns the default value, if
        ...
        Note that you can also access environment1 variables directly using
    ...
    def set_environment_variable(self, name, value):
        """Sets an environment1 variable to a specified value.
    ...
    def append_to_environment_variable(self, name, *values, **config):
        """Appends given ``values`` to environment1 variable ``name``.

        If the environment1 variable already exists, values are added after it,
        and otherwise a new environment1 variable is created.
    ...
    def remove_environment_variable(self, *names):
        """Deletes the specified environment1 variable.

        Does nothing if the environment1 variable is not set.
    ...
    def environment_variable_should_be_set(self, name, msg=None):
        """Fails if the specified environment1 variable is not set.
    ...
    def environment_variable_should_not_be_set(self, name, msg=None):
        """Fails if the specified environment1 variable is set.
    ...
    def get_environment_variables(self):
        """Returns currently available environment1 variables as a dictionary.

        Both keys and values are decoded to Unicode using the system encoding.
        Altering the returned dictionary has no effect on the actual environment1
        variables.
    ...
    def log_environment_variables(self, level='INFO'):
        """Logs all environment1 variables using the given log level.
